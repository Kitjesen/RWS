...
def main():
    ...
    controller_cfg = GimbalControllerConfig(
        yaw_pid=PIDConfig(
            kp=15.0, ki=0.5, kd=0.3,
            integral_limit=30.0, output_limit=180.0,
    ...
    print(f"{'时间':<6} {'状态':<8} {'目标位置':<20} {'误差(deg)':<20} {'云台角度(deg)':<20} {'命令(dps)':<20}")
    print("-"*110)
    ...
    while ts < duration:
        ...
        if step_count % 10 == 0:
            ...
            if events:
                ...
                print(f"{ts:5.2f}  {state:<8} {target_pos:<20} {error_str:<20} {gimbal_str:<20} {cmd_str:<20}")
