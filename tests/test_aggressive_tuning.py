...
def test_aggressive(kp_yaw, kp_pitch, target_speed, lock_threshold, duration=20.0):
    ...
    controller_cfg = GimbalControllerConfig(
        yaw_pid=PIDConfig(
            kp=kp_yaw, ki=0.8, kd=0.5,
            integral_limit=50.0, output_limit=180.0,
            derivative_lpf_alpha=0.3,
            feedforward_kv=1.0,
        ),
        pitch_pid=PIDConfig(
            kp=kp_pitch, ki=0.75, kd=0.5,
            integral_limit=50.0, output_limit=180.0,
            derivative_lpf_alpha=0.3,
            feedforward_kv=0.95,
        ),
...
def main():
    ...
    m1 = test_aggressive(kp_yaw=10.0, kp_pitch=10.5, target_speed=5, lock_threshold=2.0, duration=20.0)
    results.append(("超慢+放宽", m1))

    # 测试 2：超慢目标 + 超大 Kp + 放宽条件
    print("\n[测试 2] 超慢目标 (5 px/s) + 超大 Kp (20) + 放宽 LOCK (2.5 deg)")
    m2 = test_aggressive(kp_yaw=20.0, kp_pitch=20.5, target_speed=5, lock_threshold=2.5, duration=20.0)
    results.append(("超慢+超大Kp+放宽", m2))

    # 测试 3：极慢目标 + 大 Kp + 极度放宽
    print("\n[测试 3] 极慢目标 (3 px/s) + 大 Kp (15) + 极度放宽 (3.0 deg)")
    m3 = test_aggressive(kp_yaw=15.0, kp_pitch=15.5, target_speed=3, lock_threshold=3.0, duration=20.0)
    results.append(("极慢+极度放宽", m3))

    # 测试 4：静止目标 + 标准条件
    print("\n[测试 4] 静止目标 (0 px/s) + 标准 LOCK (0.8 deg)")
    m4 = test_aggressive(kp_yaw=10.0, kp_pitch=10.5, target_speed=0, lock_threshold=0.8, duration=20.0)
    ...
    for name, metrics in results:
        print(f"{name:<20} {metrics['lock_rate']*100:>10.2f}%  "
