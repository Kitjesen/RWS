...
- [ ] **缺少架构设计文档**
  - 无架构总览图（数据流图、层间依赖关系图）。
  - 无各 Protocol 接口的使用示例与扩展指南。
  - 无配置字段完整说明文档（各字段含义、单位、取值范围、调参建议）。
  - **修复方案**：在 `docs/` 下新增 `architecture.md`、`configuration.md`。

- [ ] **缺少坐标变换数学推导文档**
  - `algebra/coordinate_transform.py` 中的像素→归一化→相机系→body系→world系变换链缺少独立的数学推导文档。
  - 变量符号、坐标系定义、旋转矩阵约定、单位等应集中说明。
  - **修复方案**：在 `docs/` 下新增 `coordinate_math.md`，附推导过程和示意图。
